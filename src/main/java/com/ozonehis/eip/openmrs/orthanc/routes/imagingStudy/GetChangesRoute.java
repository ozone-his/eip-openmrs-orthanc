...
@Setter
@Component
public class GetChangesRoute extends RouteBuilder {
    ...
    private static final String GET_CHANGES_ENDPOINT = "/changes?limit=50&since=";

    @Override
    public void configure() {
        // spotless:off
        from("direct:orthanc-get-changes-route")
                .log(LoggingLevel.INFO, "Fetching Changes from Orthanc...")
                .routeId("orthanc-get-changes-route")
                .setHeader(Constants.CAMEL_HTTP_METHOD, constant(Constants.GET))
                .setHeader(Constants.CONTENT_TYPE, constant(Constants.APPLICATION_JSON))
                .setHeader("token", method(orthancTokenProvider, "getToken"))
                .toD(orthancConfig.getOrthancBaseUrl() + GET_CHANGES_ENDPOINT
                        + "${header." + Constants.HEADER_STUDIES_SINCE + "}")
                .end();
        // spotless:on
    }
}
